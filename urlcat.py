...
import socket
### Local Variables ###
outputFilename = 'sanitas/urls_to_categ-OUTPUT.csv'  # This the logFile/csv name in relation to the script, i'm not making the output directory make sure it exists.
urlsToCategorize = 'sanitas/urls_to_categ.txt'  # This is the file we're processing, one url per line, no quotes or commas adjust path as necessary
...
logon = s.post("https://zsapi.{0}.net/api/v1/authenticatedSession".format(cloudName), json=authentication)
...
with open(urlsToCategorize) as file:
    while True:
        ...
        results = s.post("https://zsapi.{0}.net/api/v1/urlLookup".format(cloudName), headers=headers, data=json_data)
        ...
        for x in range(0, listLen - 1):
            ...
            ## LogToConsoleAndFile(workingline)
            ## workingline += 1
            LogToConsoleAndFile(csvString)
        time.sleep(2)  # pause to not over-run the API threshold of one per second
        apiCallNumber += 1
